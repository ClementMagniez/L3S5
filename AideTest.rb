...
class TestAide < Test::Unit::TestCase
    ...
    def test_resteQueHerbeLigne
    	# Dans cette grille, il n'y a qu'une seule ligne qui est à 0 tente
    	grille=Grille.new(1097,"grilles.txt");
		aide = Aide.new(grille)
		newStatutVide = StatutVide.new(VIDE)

        assert(aide.resteQueHerbeLigne != 0, "Il n'y a plus de ligne ou il ne reste que du gazon a placer")
		...
		assert(aide.resteQueHerbeLigne == 0, "Il y a une/plusieurs ligne(s) ou il reste que du gazon a placer")
    end

    def test_resteQueHerbeColonne
    	# Dans cette grille, il n'y a qu'une seule colonne qui est à 0 tente
    	grille=Grille.new(1097,"grilles.txt");
		aide = Aide.new(grille)
		newStatutVide = StatutVide.new(VIDE)
	
        assert(aide.resteQueHerbeColonne != 0, "Il n'y a plus de colonne ou il ne reste que du gazon a placer")
		...
		assert(aide.resteQueHerbeColonne == 0, "Il y a une/plusieurs colonne(s) ou il reste que du gazon a placer")
    end
